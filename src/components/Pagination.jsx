...
setPage(pagination[0].startIndex)
    ...
    return <div className="flex flex-col items-center max-w-[700px] ">
        ...
        <div className="flex gap-3 text-[#4285f4] text-sm">
            {pagination.map((p)=>(
                 <span key={p.page} onClick={()=>paginationClickHandler(p.startIndex)} className={`cursor-pointer ${page===p.startIndex ? "text-black": " "}`}>
                    {p.page}
                 </span>   
            ))}
        </div>
